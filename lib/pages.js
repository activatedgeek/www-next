...
export async function getPageByPath(relPath) {
  ...
  const bibPath = path.join(
    ...
    `${path.basename(relPath).split(".").slice(0, -1).join()}.bib`
  )

  const mdxSource = await serialize(rawContent, {
    scope: frontmatter,
    mdxOptions: {
      remarkPlugins: [remarkEmoji, remarkGfm, remarkHint, remarkMath],
      rehypePlugins: [
        rehypeFigure,
        [
          rehypeCitation,
          {
            ...
            bibliography: (await fileExists(bibPath)) ? bibPath : null,
